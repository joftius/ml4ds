...
## Warm-up: truncated normal

- Generate some `rnorm` data
- Truncate it above/below/both
- Plot histogram of truncated data
- Add conditional density to the plot
...
Write code to use the `selectiveInference` package to compute p-values for a single simulated dataset. Compare these p-values to the same from the `summary` of OLS on the active variables. Run the code several times to get a sense for the 
